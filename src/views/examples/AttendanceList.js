...
import { fetchAttendances, createAttendance, updateAttendance } from "../../api/attendanceAPI";
...
const AttendanceList = () => {
  ...
  const handleStatusUpdate = async (employee, status, hoursWorked, reason) => {
    try {
      ...
      if (existingAttendance) {
        await axios.put(
          `http://51.210.241.36:5000/api/attendances/${existingAttendance._id}`,
          { status, hours_worked: hoursWorked, reason }
        );
      } else {
        await axios.post("http://51.210.241.36:5000/api/attendances", {
  ...
  const handleOpen = async (employee) => {
    try {
      const response = await axios.get(`http://51.210.241.36:5000/api/employees/${employee._id}`);
